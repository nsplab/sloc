...
public:
    ...
    Octree<T,AS>& operator=(Octree<T,AS> o); // XXX: typo? shouldn't it be Octree<T,AS>& o?
...
private:
    ...
    void zSliceRecursive(Array2D<T> slice, const Node* node, int size, int x, int y, int z, int targetZ) const;
...
protected:
    ...
    public:
        ...
        friend void Octree<T,AS>::deleteNode(Node** node);
    private:
        ~Aggregate() {}; // XXX: wtf is a private destructor?
    private:
        T value_[AS][AS][AS];
    };
