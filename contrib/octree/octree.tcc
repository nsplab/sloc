 ...
 *
 * \remarks Memory access is faster when \a varies the quickest, followed by
...
const T& Octree<T,AS>::at(int x, int y, int z) const
{
    ...
    while (size != aggregateSize_)
    {
        ...
        {
            size /= 2;
            n = reinterpret_cast<Branch*>(n)->child(!!(x & size), !!(y & size), !!(z & size));
        }
        else
        {
 ...
 * inadvertantly create new nodes, use the at() function.
 *
 * \see at()
 */
template <typename T, int AS>
 ...
 * <code>at(x,y,z)</code> will return the value returned by
 * emptyValue().
 *
...
void Octree<T,AS>::eraseRecursive(Node** node, int size, int x, int y, int z)
{
    ...
    if (size != aggregateSize_)
    {
        if ((*node)->type() == BranchNode)
        {
            ...
            eraseRecursive(&b->child(!!(x & size), !!(y & size), !!(z & size)), size, x, y, z);
            for (int i = 0; i < 8; ++i)
            {
                if (b->child(i)) return;
            }
    ...
    for (int i = 0; i < 8; ++i)
    {
        if (b.child(i))
        {
            ...
            case LeafNode:
                ...
                break;
            case AggregateNode:
                child(i) = new Aggregate(*reinterpret_cast<const Aggregate*>(b.child(i)));
                break;
            }
        }
        else
 ...
 * array. If no node exists for the given index, the value returned by
 * emptyValue() will be written instead.
 *
 * \remarks This method ought to be relatively fast as long as the time
 ...
 */
template <typename T, int AS>
void Octree<T,AS>::zSliceRecursive(Array2D<T> slice, const Node* node, int size, int x, int y, int z, int targetZ) const
{
    if (!node)
    {
    ...
    else if (node->type() == BranchNode)
    {
        ...
        for (int i = 0; i < 2; ++i)
            for (int j = 0; j < 2; ++j)
                zSliceRecursive(slice.subarray(i*size, j*size, (i+1)*size, (j+1)*size),
                                reinterpret_cast<const Branch*>(node)->child(j, i, !!(targetZ & size)),
                                size, x, y, z, targetZ);
    }
    else if (node->type() == AggregateNode)
    {
        for (int i = 0; i < slice.M(); ++i)
            for (int j = 0; j < slice.N(); ++j)
                slice(i,j) = reinterpret_cast<const Aggregate*>(node)->value(j, i, targetZ - z & (size-1));
    }
    else
    {
        assert(node->type() == LeafNode);
