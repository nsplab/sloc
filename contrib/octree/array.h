...
public:
    ...
    Array<T,N> subarray(const TinyVector<int,N>& begin, const TinyVector<int,N>& end);
...
template <typename T, int N>
Array<T,N> Array<T,N>::subarray(const TinyVector<int,N>& begin, const TinyVector<int,N>& end)
{
    Array<T,N> sub;
    sub.data_ = data_;
