...
make_dipoles \
    -v sphere.dipoles.vtk -o sphere.dipoles \
    0.0,0.0,0.0/0,0,1

if [ ! -f sphere.electrodes ]; then
    select_electrodes \
        -m sphere.mesh -i sphere.mat \
        -v sphere.electrodes.vtk -o sphere.electrodes \
        1/4
fi
...
set surface_mesh = sphere_h4.mesh
set surface_mesh_materials = sphere_h4.mat
set material_data = sphere_h4.sigma
set dipole_sources = sphere.dipoles

set output_vtk = sphere.vtk
set output_phi = sphere.phi.dat
DOC
