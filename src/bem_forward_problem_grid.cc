#include "sloc/bem_forward_problem_grid.h"
...
BEM_Forward_Problem_G::BEM_Forward_Problem_G(const Parameters& parameters)
...
BEM_Forward_Problem_G::~BEM_Forward_Problem_G()
{
}

void BEM_Forward_Problem_G::run()
{
    ...
    if (rank == 0)
    {
        const float init_x =  -43.75305;
        const float init_y = -154.17490;
        const float init_z = -137.75911;

        const float dist = 3.818119;

        const size_t grid_size = 11; // +1

        for (size_t x=0; x<grid_size; x++)
            for (size_t y=0; y<grid_size; y++)
                for (size_t z=0; z<grid_size; z++) {
                    for (size_t canon=0; canon<3; canon++) {
                        stringstream ss;
                        float current_x=0.0;
                        float current_y=0.0;
                        float current_z=0.0;
                        ...
                        float node_x = float(x) * dist + init_x;
                        float node_y = float(y) * dist + init_y;
                        float node_z = float(z) * dist + init_z;
                        dipole_sources.clear();
                        dipole_sources.add_source(dealii::Point<3>(node_x, node_y, node_z), dealii::Point<3>(current_x, current_y, current_z));

                        this->assemble_rhs();
                        sloc::BEM_Forward_Problem::solve_system();

                        parameters.output_phi = ss.str();
...
void BEM_Forward_Problem_G::assemble_rhs()
{
...
void BEM_Forward_Problem_G::assemble_range_contrib(unsigned int cv_begin, unsigned int cv_end, std::valarray<double>& contrib)
{
...
void BEM_Forward_Problem_G::assemble_system_from_contrib(unsigned int cv_begin, unsigned int cv_end, std::valarray<double>& contrib, const int k)
{
    const unsigned int n_dofs = mf.nb_basic_dof();
    const unsigned int fe_dofs_per_cell = mf.nb_basic_dof_of_element(0);
    assert(fe_dofs_per_cell == 3);
...
void BEM_Forward_Problem_G::assemble_system()
{
    using namespace dealii;
