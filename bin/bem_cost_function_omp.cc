/* bem_cost_function_omp.cc
 *
 * Evaluate forward solution's cost function in a cloud of points around the true solution using OpenMP.
 *
 */
#include <iostream>
#include <fstream>
...
public:
	...
	cout<<"after pinv(L) * phi_measured"<<endl;
        ...
        matrix_out.build_patches(L_pinv, "L_pinv");
        ostringstream oss;
        oss<<"L_pinv_"<<pt[0]<<"_"<<pt[1]<<"_"<<pt[2];
        ofstream outFile(oss.str().c_str());
        matrix_out.write_vtu(outFile);
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        const int nx = parameters.grid_dims[0];
        const int ny = parameters.grid_dims[1];
        const int nz = parameters.grid_dims[2];
        ...
        float outputData[nx][ny][nz][7];

        omp_set_num_threads(2);
        #pragma omp parallel for  //firstprivate(cost_function)
        for (i = 0; i < nx; i++)
        {
            cout<<"i: "<<i<<endl;
            double x = X[i];
            for (int j = 0; j < ny; j++)
            {
                ...
                for (int k = 0; k < nz; k++)
                {
                    ...
                    outputData[i][j][k][0] = pt[0]; outputData[i][j][k][1] = pt[1]; outputData[i][j][k][2] = pt[2];
                    outputData[i][j][k][3] = dp[0]; outputData[i][j][k][4] = dp[1]; outputData[i][j][k][5] = dp[2];
                    outputData[i][j][k][6] = cost;
                }
            }
        }
        ...
        for (i = 0; i < nx; i++)
            for (j = 0; j < ny; j++)
                for (k = 0; k < nz; k++)
                    out << outputData[i][j][k][0] << " " << outputData[i][j][k][1] << " " << outputData[i][j][k][2] << " "
                        << outputData[i][j][k][3] << " " << outputData[i][j][k][4] << " " << outputData[i][j][k][5] << " "
                        << outputData[i][j][k][6] << endl;
        out.close();
